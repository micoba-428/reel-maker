...
import config
from batch_producer import BatchProducer, load_pool, zip_outputs
...
<style>
  ...
  .pool-status { font-size: 0.9rem; color: #aaa; }
  .pool-status .ok { color: #1aaa55; }
  .pool-status .ng { color: #ff5078; }
</style>
...
# ── プール状況確認 ────────────────────────────────────────────────────────────
pools = {n: load_pool(os.path.join(config.POOL_BASE, f"pool_{n}")) for n in range(1, 6)}
...
with c1:
    ...
    st.markdown(
        ...
        f'<div class="pool-status">全プール使用<br>5クリップ・素材豊富向け</div>'
...
with c2:
    ...
    st.markdown(
        ...
        f'<div class="pool-status">Pool 1を2回使用<br>4クリップ・素材少なめ向け</div>'
        f'</div>', unsafe_allow_html=True,
    )
...
# ── 必須プールチェック ────────────────────────────────────────────────────────
required_for_pattern = list(set(int(c) for c in p))
missing = [n for n in required_for_pattern if counts[n] == 0]
if missing:
    st.warning(f"⚠️ Pool {missing} に写真がありません。振り分けを完了してください。")

st.markdown("---")

# ── リール長さ ───────────────────────────────────────────────────────────────
st.markdown("### ⏱️ リールの長さ")
length_sec = st.select_slider(
    "全体の長さ（秒）",
...
if st.button(f"🎬 {num_reels}本 生成する", disabled=not can_gen):
    try:
        ...
        # パターンに合わせてプラン作成
        plan = []
        for day in range(1, num_reels + 1):
            clips = []
            for pos_str in p:
                pool_num = int(pos_str)
                pool_files = pools[pool_num]
                if pool_files:
                    clips.append(pool_files[(day - 1) % len(pool_files)])
            plan.append({"day": day, "clips": clips})

        out_dir = os.path.join(config.OUTPUT_DIR, "reels")
        ...
        for i, day_plan in enumerate(plan):
            day = day_plan["day"]
            status.info(f"🎬 {day}本目を生成中... ({i+1}/{num_reels})")
            reel_bar = st.empty()
        ...
        status.success(f"🎉 完了！ {num_reels}本のリールを生成しました")
        st.markdown(f"**保存先:** `{out_dir}`")
